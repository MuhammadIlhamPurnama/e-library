...
const FavoritePage = () => {
  ...
  const fetchData = async (pageNumber = 1) => {
    try {
      setLoading(true)
      let url = `/favorites?page=${pageNumber}`
  ...
  return (
    <section className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-800">Favorited Books</h2>
          ...
          {favorites && favorites.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 cursor-pointer">
              {favorites.map((favorite) => (
                <NavLink key={favorite.id} to={`/books/${favorite.Book.id}`}>
                  <div className="bg-white rounded-md p-4 flex flex-col items-center hover:scale-110 duration-300">
                    {favorite.Book.imageUrl ? (
                      <img
                        src={favorite.Book.imageUrl}
                        alt={favorite.Book.bookName}
                    ...
                    <h3 className="font-semibold text-sm text-gray-900 line-clamp-2 pb-2">{favorite.Book.bookName}</h3>
            ...
            <p className="text-gray-500">No favorited books found.</p>
          )}
